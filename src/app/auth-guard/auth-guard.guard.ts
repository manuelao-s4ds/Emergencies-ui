...
export class AuthGuard implements CanActivate {
  ...
  canActivate(
    ...
    const accessCode = this.tokenHandlerService.getAccessCode();
    if (accessCode) {
      return this.tokenHandlerService.exchangeToken(accessCode).map((token) => true);
    }
    location.href = 'http://localhost:3000/oauth2/authorize?client_id=emergency&response_type=code&redirect_uri=http://localhost:4200';
  }
